Feature: Lazy Contextual LHS Population
  ...
  Scenario: Force recalculate bypasses both memory and storage cache
    Given contextual LHS cache exists in memory for hour 12
    And the cached value is 3.0 degrees per hour
    When ensure_contextual_lhs_populated is called for hour 12 with force_recalculate=True
    Then LHS should be recalculated from cycles
    And new value should replace memory cache
    And new value should replace storage cache
    And old cached value should be discarded
