...
class TestHeatingCycleLifecycleManager:
    ...
    # ===== P3 Tests: cache-first behavior & cycle persistence =====

    @pytest.mark.asyncio
    async def test_refresh_skips_extraction_when_cache_fully_covered(
        self,
        manager: HeatingCycleLifecycleManager,
        mock_heating_cycle_service: Mock,
        mock_heating_cycle_storage: Mock,
        ...
        """P3-1: refresh_heating_cycle_cache must NOT launch extraction when cache covers all days.

        Scenario: the IHeatingCycleStorage already holds a cycle that spans the
        entire retention window.  _find_missing_date_ranges() should find zero
        missing days → no RecordingExtractionQueue should be created.

        This test would have FAILED before the cache-aware scheduling was
        introduced in _find_missing_date_ranges() / _launch_extraction_for_ranges().
        """
        import datetime as dt_module

        retention = manager._device_config.lhs_retention_days  # 30
        now = dt_module.datetime.now(tz=dt_module.timezone.utc)

        # GIVEN: one cycle spanning the entire retention window (all days covered)
        # Use the REAL current time so _calculate_extraction_window() aligns
        window_start = now - timedelta(days=retention)
        covering_cycle = self._create_heating_cycle(
            start_time=window_start,
            duration_hours=24 * retention,  # spans the whole window
        )

        full_cache = HeatingCycleCacheData(
            device_id=manager._device_config.device_id,
            cycles=(covering_cycle,),
            last_search_time=now,
            retention_days=retention,
        )
        mock_heating_cycle_storage.get_cache_data.return_value = full_cache

        # WHEN: refresh is called
        with suppress(Exception):
            await manager.refresh_heating_cycle_cache()

        # THEN: no extraction queue was launched (cache was already complete)
        assert manager._extraction_queue is None
        mock_heating_cycle_service.extract_heating_cycles.assert_not_called()

    @pytest.mark.asyncio
    async def test_on_cycles_extracted_persists_via_append_cycles(
        self,
        manager: HeatingCycleLifecycleManager,
        mock_heating_cycle_storage: Mock,
        mock_lhs_storage: Mock,
        ...
        """P3-2: _on_cycles_extracted() must persist cycles via heating_cycle_storage.append_cycles().

        The old implementation tried to call lhs_storage.cache_heating_cycle()
        which does NOT exist on ILhsStorage/HALhsStorage, so cycles were
        silently never persisted.  After the P1 fix, heating_cycle_storage
        .append_cycles() must be called and lhs_storage must NOT be called
        for persistence.

        This test would have FAILED before the P1 fix because the old code
        only called cache_heating_cycle() (which is not a real ILhsStorage method).
        """
        test_cycles = [
            self._create_heating_cycle(base_datetime - timedelta(hours=2)),
            self._create_heating_cycle(base_datetime - timedelta(hours=5)),
        ]

        # WHEN: extraction callback is triggered with cycles
        await manager._on_cycles_extracted(test_cycles)

        # THEN: cycles are persisted via heating_cycle_storage.append_cycles()
        mock_heating_cycle_storage.append_cycles.assert_called_once()
        call_args = mock_heating_cycle_storage.append_cycles.call_args
        assert call_args[0][0] == manager._device_config.device_id
        assert call_args[0][1] == test_cycles

        # AND: the non-existent lhs_storage.cache_heating_cycle is NOT used for persistence
        # (verifies the dead-code path was removed)
        assert not hasattr(mock_lhs_storage, "cache_heating_cycle") or \
            not mock_lhs_storage.cache_heating_cycle.called

    @pytest.mark.asyncio
    async def test_find_missing_date_ranges_skips_cached_days(
        self,
        manager: HeatingCycleLifecycleManager,
        mock_heating_cycle_storage: Mock,
        base_datetime: datetime,
    ) -> None:
        """P3-3: _find_missing_date_ranges() must exclude days already covered by cache.

        Scenario: retention window is [D-5, D-1] (5 days).  The cache covers
        D-4 and D-3.  Only D-5, D-2, and D-1 should be reported as missing.

        This test would have FAILED before the incremental scheduling logic
        was implemented (old code always returned the full window as missing).
        """
        now = base_datetime
        retention = 5

        # GIVEN: device config with 5-day retention
        from custom_components.intelligent_heating_pilot.domain.interfaces.device_config_reader_interface import (
            DeviceConfig,
        )
        manager._device_config = DeviceConfig(
            device_id=manager._device_config.device_id,
            vtherm_entity_id=manager._device_config.vtherm_entity_id,
            scheduler_entities=manager._device_config.scheduler_entities,
            lhs_retention_days=retention,
        )

        window_start = (now - timedelta(days=retention)).date()
        window_end = (now - timedelta(days=1)).date()

        # Cache covers D-4 and D-3 (the middle two days)
        cycle_d4 = self._create_heating_cycle(now - timedelta(days=4))   # D-4
        cycle_d3 = self._create_heating_cycle(now - timedelta(days=3))   # D-3

        partial_cache = HeatingCycleCacheData(
            device_id=manager._device_config.device_id,
            cycles=(cycle_d4, cycle_d3),
            last_search_time=now,
            retention_days=retention,
        )
        mock_heating_cycle_storage.get_cache_data.return_value = partial_cache

        # WHEN: find missing date ranges
        missing = await manager._find_missing_date_ranges(window_start, window_end)

        # THEN: D-4 and D-3 must be absent from all missing ranges
        ...
        assert (now - timedelta(days=4)).date() not in missing_days, "D-4 is in cache, must not be missing"
        assert (now - timedelta(days=3)).date() not in missing_days, "D-3 is in cache, must not be missing"
        # D-5, D-2, D-1 must still be missing
        assert (now - timedelta(days=5)).date() in missing_days
        assert (now - timedelta(days=2)).date() in missing_days
        assert (now - timedelta(days=1)).date() in missing_days
