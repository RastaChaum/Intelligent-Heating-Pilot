...
class LHSCalculationService:
    ...
    def calculate_simple_average(self, slope_values: list[float]) -> float:
        ...
        if not slope_values:
            _LOGGER.debug(
                "No slope values provided, using default: %.2f°C/h", DEFAULT_HEATING_SLOPE
            )
        ...
        _LOGGER.debug(
            "Calculated simple average from %d values: %.2f°C/h", len(slope_values), avg_slope
        )
    ...
    def calculate_global_lhs(self, heating_cycles: list[HeatingCycle]) -> float:
        ...
        if not heating_cycles:
            _LOGGER.debug(
                "No heating cycles provided, using default: %.2f°C/h", DEFAULT_HEATING_SLOPE
            )
            return DEFAULT_HEATING_SLOPE
        ...
        _LOGGER.info(
            "Calculated global LHS from %d cycles: %.2f°C/h", len(heating_cycles), avg_slope
        )

        return avg_slope

    def calculate_contextual_lhs(
        self, heating_cycles: list[HeatingCycle], target_hour: int
    ) -> float:
        ...
        if not active_cycles:
            ...
            _LOGGER.debug("No cycles active at hour %d, using global: %.2f°C/h", target_hour, lhs)
            return lhs
    ...
    def calculate_average_dead_time(self, heating_cycles: list[HeatingCycle]) -> float | None:
        ...
        """
        _LOGGER.debug("Calculating average dead_time from %d heating cycles", len(heating_cycles))
        ...
        # Calculate simple average
        total_dead_time = sum(
            c.dead_time_cycle_minutes
            for c in cycles_with_dead_time
            if c.dead_time_cycle_minutes is not None
        )
